...
for st = 1:3                                            % Starts a loop to train the model for 3 sets of data (or outputs).
    ...
    msex = calc_rmse(y(:,st), X * w_rls);            % Computes the root mean square error of the predictions.
...
% Performs prediction on test data using the trained model gain.
Test = testPrediction(srcX(sample+1:sample+testSample,:), srcY(sample+1:sample+testSample,:), 0, B, libr); 
